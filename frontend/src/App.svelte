<script lang="ts">
  ...
  import { setSettingsState } from '$lib/state/settings.svelte';
  ...
  let settings = setSettingsState();
  setScriptState();

  $effect(() => {
    if (settings.settings?.ui?.color) {
      const { color, theme } = settings.settings.ui;
...
<div class="window-wrapper">
  ...
  <div
    ...
    class:full-closed={!sidebar.isOpen && settings.settings?.ui?.fullyCloseSidebar}
  >
    <Sidebar />
...
<style>
  ...
  .main-content {
    padding: var(--space-2);
    overflow-y: auto;
    background-color: light-dark(var(--color-background, #fff), var(--neutral-950-tint, #121212));
    background-image: light-dark(
      none,
      radial-gradient(
        circle at 80% 80%,
        color-mix(in srgb, var(--color-brand, #3b82f6) 10%, transparent),
        transparent 45%
      )
    );
