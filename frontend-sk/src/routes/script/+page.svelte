	...
	<Card style="height: 100%; overflow: hidden;">
		<div class="script-sidebar">
			<div class="tree-viewport">
				<Tree
					items={treeItems}
					activeId={currentScriptName}
					onSelect={(node) => (currentScriptName = node.id)}
				/>
			</div>
			...
			<div class="editor-layout">
				<div class="editor-toolbar">
					...
					{#if currentScript.isEnabled}
						<Button variant="error" icon={Trash} onclick={() => (currentScript.isEnabled = false)}
							>Disable</Button
						>
					{:else}
						<Button variant="success" icon={Check} onclick={() => (currentScript.isEnabled = true)}
							>Enable</Button
						>
					{/if}
					{#if currentScript.event == EventKey.ManualInvoke}
						<Button variant="accent" onclick={async () => await invokeScriptTest()}
							>Test Script</Button
						>
					{/if}
				</div>
